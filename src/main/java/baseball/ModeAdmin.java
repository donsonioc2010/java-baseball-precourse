...
public class ModeAdmin {
    ...
    public boolean getRestartVal() {
        boolean forFirstWhile = false;
        String runMode = null;
        while(!forFirstWhile){
            System.out.print(Constants.PRT_REGAME);
            runMode = Console.readLine().trim();
            forFirstWhile = validationModeVal(runMode);
        }
        System.out.println();
    ...
    private boolean confirmValue(String runMode) {
        if(Constants.REGAME_VAL.equals(runMode)) {
            return true;
        }
        return false;
    }
    ...
    private boolean validationModeVal(String runMode) {
